...
namespace AufenCode
{
    ...
    public static class RoyaltyCompatBootstrap
    {
        ...
        {
            Harmony harmony = new Harmony("AufenCode.RoyaltyCompat");
        ...
        {
            Type targetType = AccessTools.TypeByName("AufenCodeRoyalty.CompAbilityEffect_Pawnchanger");
            if (targetType == null)
            {
            ...
            MethodInfo applyMethod = AccessTools.Method(targetType, "Apply", new[] { typeof(LocalTargetInfo), typeof(LocalTargetInfo) })
        ...
        {
            Type targetType = AccessTools.TypeByName("AufenCodeRoyalty.CompAbilityEffect_SpidersOmg");
            if (targetType == null)
            {
                Log.Message("[AufenCode/RoyaltyCompat] SpidersOmg type not found. Skipping.");
                return;
            }
            MethodInfo applyMethod = AccessTools.GetDeclaredMethods(targetType).FirstOrDefault(m => m.Name == "Apply");
            if (applyMethod == null)
            {
        ...
        {
            Type targetType = AccessTools.TypeByName("AufenCodeRoyalty.CompAbilityEffect_Boomcast");
            if (targetType == null)
            {
        ...
        private static void LogPawnStripPresence()
        {
            try
            {
                List<MethodInfo> methods = AccessTools.GetDeclaredMethods(typeof(Pawn)).Where(m => m.Name == "Strip").ToList();
                if (methods.Count == 0)
                {
        ...
        {
            MethodInfo stripCompat = AccessTools.Method(typeof(RoyaltyCompatBootstrap), nameof(StripPawn));
        ...
        {
            MethodInfo compat = AccessTools.Method(typeof(RoyaltyCompatBootstrap), nameof(GeneratePawnCompat));
            foreach (CodeInstruction ci in instructions)
            {
        ...
        {
            MethodInfo compat = AccessTools.Method(typeof(RoyaltyCompatBootstrap), nameof(DoExplosionCompat));
            foreach (CodeInstruction ci in instructions)
            {
                if ((ci.opcode == OpCodes.Call || ci.opcode == OpCodes.Callvirt) && ci.operand != null)
                {
        ...
        {
            if (!(__exception is MissingMethodException mme) ||
                !(mme.Message?.Contains("PawnGenerator.GeneratePawn") ?? false)) return __exception;
        ...
        {
            if (!(__exception is MissingMethodException mme) ||
                !(mme.Message?.Contains("GenExplosion.DoExplosion") ?? false)) return __exception;
        ...
        public static Pawn GeneratePawnCompat(PawnKindDef kindDef, Faction faction)
        {
            try
            {
                ...
                ConstructorInfo reqCtor = AccessTools.Constructor(typeof(PawnGenerationRequest), new[] { typeof(PawnKindDef), typeof(Faction), typeof(PawnGenerationContext), typeof(int), typeof(bool) });
                if (reqCtor != null)
                {
                    PawnGenerationRequest req = (PawnGenerationRequest)reqCtor.Invoke(new object[] { kindDef, faction, PawnGenerationContext.NonPlayer, -1, true });
            ...
            try
            {
                ...
                MethodInfo method = AccessTools.GetDeclaredMethods(typeof(PawnGenerator)).FirstOrDefault(m => m.Name == "GeneratePawn");
                if (method != null)
                {
                    ...
                    if (method.Invoke(null, args) is Pawn pawn) return pawn;
                }
            }
            catch
            {
        ...
        public static void DoExplosionCompat(IntVec3 center, Map map, float radius, DamageDef damType, Thing instigator = null)
        {
            try
            {
                ...
                MethodInfo method = AccessTools.GetDeclaredMethods(typeof(GenExplosion)).FirstOrDefault(m => m.Name == "DoExplosion");
                if (method != null)
                {
                    ParameterInfo[] ps = method.GetParameters();
                    object[] args = new object[ps.Length];
                    for (int i = 0; i < ps.Length; i++)
                    {
                        ...
                        else if (pt == typeof(SoundDef) || pt == typeof(ThingDef)) args[i] = null;
            ...
            // Fallback to the current signature subset if known
        ...
        public static void StripPawn(Pawn pawn)
        {
            ...
            try
            {
                ...
                List<Apparel> worn = apparelTracker?.WornApparel?.ToList();
                if (worn != null)
                {
                    foreach (Apparel ap in worn.Where(ap => ap != null).Where(ap => !TryInvokeApparelTryDrop(apparelTracker, ap, pos, map)))
                    {
                ...
                if (eq != null)
                {
                    ...
                    {
                        List<ThingWithComps> all = eq.AllEquipmentListForReading?.ToList();
                        if (all != null)
                        {
                            foreach (ThingWithComps e in all.Where(e => e != null).Where(e => !TryInvokeTryDropEquipment(eq, e, pos)))
                            {
                                eq.Remove(e);
                                if (!e.Spawned)
                                {
        ...
        {
            IEnumerable<MethodInfo> methods = AccessTools.GetDeclaredMethods(apparelTracker.GetType()).Where(m => m.Name == "TryDrop");
            foreach (MethodInfo m in methods)
            {
                ...
                try
                {
                    object result = m.Invoke(apparelTracker, args);
        ...
        {
            MethodInfo m = AccessTools.Method(equipmentTracker.GetType(), "DropAllEquipment");
            ...
            for (int i = 0; i < ps.Length; i++)
            {
                ...
                else if (pt == typeof(Map)) args[i] = (equipmentTracker as Pawn_EquipmentTracker)?.pawn?.MapHeld;
                else if (pt == typeof(ThingPlaceMode)) args[i] = ThingPlaceMode.Near;
                else args[i] = null;
            }
        ...
        {
            IEnumerable<MethodInfo> methods = AccessTools.GetDeclaredMethods(equipmentTracker.GetType()).Where(mm => mm.Name == "TryDropEquipment");
            foreach (MethodInfo m in methods)
            {
                ParameterInfo[] ps = m.GetParameters();
                object[] args = new object[ps.Length];
                ...
                try
                {
                    object result = m.Invoke(equipmentTracker, args);
                    if (result is bool b && b) return true;
                }
                catch
                {
